...
__global__ void medianFilter(float* data, int N)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;

    if (idx >= 1 && idx <= N - 2)
    {
        data[idx] = fmaxf(fminf(data[idx - 1], data[idx]), fminf(fmaxf(data[idx - 1], data[idx]), data[idx + 1]));
    }
}
...
int main()
{
    ...
    {
        medianFilter<<<grid, block>>>(d_data, length);
